...
the current code and configuration, after every stage it reads from, with
exactly its declared outputs, each unchanged since; a skipped stage must be
one allowed to skip, leave none of its outputs behind, and take every stage
that reads it along.
"""
...
STAGES: dict[str, tuple[tuple[str, ...], list[str]]] = {
    ...
    "mentions": (("prepare", "identity", "network"), ["processed/mentions.parquet", "processed/mention_centrality.parquet",
                                                      "results/mention_resolution.json"]),
...
FIELDS = {"seq", "skipped", "code_sha256", "config_sha256", "outputs"}
...
def stale_reasons(config: dict, stages: list[str] | None = None) -> list[str]:
    ...
    `stages` limits the check to those stages (and what they read from); all by default.
    """
    path = _manifest_path(config)
    if not path.exists():
    ...
    code, configuration = code_hash(), config_hash(config)
    reasons = []
