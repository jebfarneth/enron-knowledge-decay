...
Computes employee-level organizational knowledge risk and departure-impact
metrics from the Enron knowledge graph and expertise profiles.

The simulation removes high-centrality employees from the communication graph,
measures structural disruption, estimates topic-level replacement coverage,
and produces risk-score outputs used by the dashboard export layer.
...
Usage:
    python pipeline/decay_simulation.py

Notes:
    This script is intentionally deterministic except for NetworkX approximate
    betweenness sampling. It is part of the analytical pipeline behind the
    12-month recovery and knowledge-loss views in the dashboard.
"""
...
def main():
    ...
    print(f"  Approximate betweenness centrality (k={BETWEENNESS_SAMPLE_SIZE}, may take a few minutes)...")
    t0 = time.time()
    betweenness = pd.Series(
        nx.betweenness_centrality(G, k=BETWEENNESS_SAMPLE_SIZE, normalized=True, weight="weight"),
        name="betweenness",
    )
    print(f"  Betweenness done in {time.time() - t0:.1f}s")

    # ── 4. Simulate departure of top 200 highest-degree nodes ─────────────────────
    print(f"\n[4/7] Simulating departure of top {TOP_N_EMPLOYEES} highest-degree nodes...")

    top200 = sorted(G.nodes(), key=lambda n: G.degree(n), reverse=True)[:TOP_N_EMPLOYEES]
    ...
    for idx, node in enumerate(top200):
        if idx % 25 == 0:
            print(f"  [{idx+1:3d}/{TOP_N_EMPLOYEES}] simulating {node}")
